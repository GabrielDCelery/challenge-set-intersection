...
- Key column selection must be configurable: the algorithm must produce correct results when `--key-columns` names any valid column, not just `udprn`.
...
| data/A_f.csv vs data/B_f.csv with `--key-columns udprn`        | E2E         | Primary acceptance test — real input files, named column selection                |
| data/A_f.csv vs data/B_f.csv with no `--key-columns` flag      | E2E         | Confirms the program exits with a clear error and non-zero exit code — no default |
...
| `--key-columns` names a column that does not exist in the file | Integration | Ensures clear error message and non-zero exit code                                |
